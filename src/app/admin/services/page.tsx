...
export default function AdminServicesPage() {
  ...
  const fetchServices = async () => {
    try {
      ...
      const url = `${process.env.NEXT_PUBLIC_API_URL}/admin/services?${queryParams.toString()}`;
  ...
  const updateStatus = async (id: string, status: 'approved' | 'rejected') => {
    ...
    try {
      await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/admin/services/${id}/status`, { status }, { withCredentials: true });
      setServices(prev =>
        status === 'approved'
